...
void EmbyClient::setServer(const QString &url) {
    ...
    if (!u.isValid() || (u.scheme() != "http" && u.scheme() != "https")) {
        qWarning() << "EmbyClient::setServer: rejected invalid URL:" << url;
        return;
    }
    m_serverUrl = u.toString(QUrl::StripTrailingSlash);
...
void EmbyClient::login(const QString &username, const QString &password) {
    ...
    QString auth = QStringLiteral("MediaBrowser Client=\"%1\", Device=\"Desktop\", DeviceId=\"%2\", Version=\"%3\"")
                       .arg(MFPLAYER_APP_NAME, m_deviceId, MFPLAYER_VERSION);
...
QByteArray EmbyClient::authHeader() const {
    ...
    parts << QStringLiteral("DeviceId=\"%1\"").arg(m_deviceId);
    parts << QStringLiteral("Version=\"%1\"").arg(MFPLAYER_VERSION);
