...
struct mpv_vulkan_init_params {
  ...
  unsigned num_enabled_extensions;
};
#endif
}
...
MpvController::MpvController(QObject *parent) : QObject(parent) {
  ...
  // auto-safe: 仅启用无已知 bug 的硬解后端 (Windows→d3d11va),
  // 失败自动回落软解。 gpu-next 下解码输出零拷贝直通 libplacebo,
  // HDR/tone-mapping 管线不受影响。
  ...
  mpv_set_option_string(m_mpv, "stream-lavf-o",
                        "timeout=30000000,reconnect=1,reconnect_at_eof=1,"
                        "reconnect_streamed=1,reconnect_delay_max=5");
  mpv_set_option_string(m_mpv, "force-seekable", "yes");

  mpv_set_option_string(m_mpv, "user-agent", MFPLAYER_USER_AGENT);

  // Route mpv log messages to Qt debug output (visible in DebugView / Qt
...
bool MpvController::ensureRenderCtx(QQuickWindow *window) {
  ...
  if (rhi->backend() == QRhi::Vulkan) {
    ...
    // Tell libmpv what the device was actually created with, so libplacebo
    // only uses genuinely enabled features. Null when Qt created the device
    // itself (takeover unsupported) — libmpv then degrades to a 1.2 level.
    vkp.enabled_features = VulkanDevice::enabledFeatures2();
    vkp.enabled_extensions = VulkanDevice::enabledExtensions();
    vkp.num_enabled_extensions = VulkanDevice::enabledExtensionCount();
    ...
    params[i++] = {MPV_RENDER_PARAM_BACKEND, const_cast<char *>("gpu-next")};
    params[i++] = {MPV_RENDER_PARAM_ADVANCED_CONTROL, &advanced};
  ...
  // Pass X11/Wayland display for hardware decoding interop and color
  // management.
#ifndef Q_OS_WIN
  auto *pn = QGuiApplication::platformNativeInterface();
  void *x11Display = pn ? pn->nativeResourceForIntegration("display") : nullptr;
  void *wlDisplay =
      pn ? pn->nativeResourceForIntegration("wldisplay") : nullptr;
  if (x11Display) {
...
void MpvController::play(const QString &url, const QString &referrer,
  ...
  if (!m_mpv)
    return;
  qDebug() << "MpvController: loading" << url
...
void MpvController::addSubtitleFile(const QString &url, const QString &title,
...
void MpvController::toggleStats() {
  if (!m_mpv)
    return;
  const char *args[] = {"script-binding", "stats/display-stats-toggle",
                        nullptr};
  mpv_command_async(m_mpv, 0, args);
}
...
void MpvController::wakeup(void *ctx) {
  QMetaObject::invokeMethod(static_cast<MpvController *>(ctx),
                            &MpvController::onMpvEvents, Qt::QueuedConnection);
}
