...
## 明确不修 (2026-08 审计的结论, 别再当 bug 提)

这三条都核实过确实成立, 权衡后判定不值得动。再被扫出来时直接引用这里的判据:

1. **OpenGL 离屏 FBO 从未绑定就附加纹理** — 只影响 OpenGL 后端。图形 API 默认自动选择,
   不会有人手动切过去。
2. **外挂字幕 `sub-add` 还在排队, 就去 track-list 里模糊匹配** — 外挂字幕基本都带名字,
   模糊匹配这条兜底路径用不上; 要修得动 `tracksChanged` 的时序, 风险大于收益。
3. **`stop()` 不排空 DB 写入队列, 主动丢弃最后一批** — 丢的只是缓存写入, 数据不会错,
   下次启动重新拉。
