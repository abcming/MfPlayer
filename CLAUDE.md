...
core/cache/     → CacheStore (SQLite+内存 双缓存 + 图片下载线程池)
...
ui/providers/   → IconProvider + ImageCacheProvider
ui/qml/
...
| 线程 | 干什么 | 安全边界 |
|------|--------|---------|
| **主线程** | 所有 QObject, CurlEngine::tick(), JSON 解析, 信号槽 | 默认运行域 |
...
| **worker threads** | 图片下载/验证/解码/过期清理 | m_imageCacheMutex / m_workerMutex / m_pixmapMutex |
...
| 机制 | 用途 | 位置 |
|------|------|------|
| 代计数器 generation | 取消过期异步回调 | PlaybackController (`m_playGeneration`), ServerManager, CacheStore |
| QPointer 守卫 | 回调前检查对象存活 | EmbyClient 全部 50+ 回调, CacheStore, CurlEngine |
...
- 媒体缓存: 内存 HashMap + SQLite 双层，内存优先，过期数据留作 fallback
- 图片加载: CachedImage → CacheStore::cachedImageUrl → ImageLoadQueue (max 3 concurrent) → ImageCacheProvider
